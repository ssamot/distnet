...
* This will form the main framework our talk - no need to understand everything
...
## No model? 
* The existance of an a-priori model is extremely unrealistic
* Three major classes of methods with dealing with this: 
    * **Model-free** 
        * Learn policy directly by roaming around the world
    * **Model based**
        * Learn policy indirectly
...
* Learning a model is most of the time MUCH harder than learning to act based on states directly
* Contradicts with our (my?) intuition
    * We make inferences about the future all time + logical reasoning, can't be that Q-values are enough!
* Doing any large-scale learning requires tons of tricks (see Deep-Q learning by deep mind)
    * Maybe we haven't found the right tricks for model-based stuff?


## Partial Observability
* It's unintuitive
    * ...because the world is not fully observable
...
* But there is no way we can get this because we don't even have samples of $T(s'|s,a)$
...
* Game of Poker: 
  * External Model: The Game itself
  * Internal Model: A probability distribution over possible opponent cards
...
* Cannot handle inputs of arbitrary length
...
* You can choose which beliefs to attend over
...
* Nothing really good out there IMHO
