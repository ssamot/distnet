...
inputX = [np.asarray(X, dtype=np.float32), np.asarray(Xq, dtype=np.float32)]
inputy = np.asarray(Y, dtype=np.float32)

model.compile(optimizer='adam', loss='categorical_crossentropy', class_mode='categorical')
print('Training')
model.fit(inputX,inputy,  batch_size=BATCH_SIZE, nb_epoch=EPOCHS, validation_split=0.05, show_accuracy=True)
